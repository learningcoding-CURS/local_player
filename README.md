...
| **手势切换（无需长按）** | 直接滑动即可切换上一首/下一首（竖屏默认上下滑，横屏默认左右滑） |
...
| **"我的列表"方块/列表双模式** | 主界面工具栏切换视图时同步作用于播放列表卡片；可配置列数（2/3/4 列）与传统列表行，均支持长按设置/清除自定义封面 |
...
| **播放列表面板外观可配置** | 设置中可调节面板背景透明度（0–100%）、宽度（左右滑出时 200–450 dp）、高度（上下滑出时 200–550 dp）、**视频方块大小（40–120 dp）**、列表排列方向（竖排/横排） |
| **播放控件全面可配置** | 设置中可控制每个播放按钮的显隐和颜色（白/红/橙/青/绿/黄），含播放界面内置设置入口 |
| **倍速按钮可配置** | 设置中可控制倍速按钮的显隐、文字颜色、**外圈有无+颜色**（白/红/橙/青/绿/黄）、顶栏排序位置 |
| **长按 2.5× 提示可配置** | 设置 → 长按 标签：可控制长按快进时中央「2.5× 快进中」图标的显隐、颜色、透明度（10–100%） |
| **跳转/切换按钮全屏偏移** | 跳转按钮（前进/后退）和切换按钮（上一首/下一首）支持 ±800 dp 上下偏移，**两行独立定位互不影响**，可在全屏任意位置放置，偏移后点击仍正常响应 |
| **按钮间距可调** | 跳转按钮（前进/后退）与播放按钮之间的间距、切换按钮（上一首/下一首）之间的间距，均可在设置中调节（竖屏 0–300 dp / 0–400 dp，横屏同） |
| **跳转/切换按钮竖屏横屏区分** | 跳转按钮和切换按钮的大小、透明度、上下偏移、按钮间距均可分别针对竖屏和横屏独立配置 |
| **跳转/切换布局优化** | 移除两行间固定间距，仅用跳转/切换各自的上下偏移控制位置；横屏左右间距上限提高（跳转 0–300 dp、切换 0–400 dp），上下偏移 ±200 dp |
...
| **方块/列表双模式** | 主界面工具栏一键切换方块视图（方格大小可调）和传统列表，带真实封面缩略图 |
| **方格大小可调** | 音视频库内音视频、内置媒体内音视频始终可见；我的列表（卡片）、列表内音视频在「列表模式」内调节（80–220 dp） |
...
| **内置媒体 / 我的列表 有无** | 开关控制底部导航是否显示「内置媒体」「我的列表」标签；关闭后对应标签隐藏 |
| 底部标签顺序 | 通过 ↑↓ 按钮调整底部导航标签的排列顺序 |
| **工具栏按钮排序** | 通过 ↑↓ 按钮调整工具栏按钮（媒体库/统计/布局切换/设置）的排列顺序 |
| 列表显示模式 | 方块视图（网格）或传统列表 |
| **方格大小** | 音视频库内音视频 / 内置媒体内音视频 始终可见可调；我的列表（卡片）/ 列表内音视频 在「列表模式」内调节（80–220 dp） |
...
| 按钮上下偏移 | -800 ~ +800 dp | 0 dp | 调整跳转按钮在播放界面中的垂直位置（正值向上）；全屏任意位置可调，偏移后点击仍有效 |
...
| **跳转按钮上下偏移** | -800 ~ +800 dp，默认 0；全屏任意位置可调，偏移后点击仍有效 |
...
| **上下偏移** | -800 ~ +800 dp | 0 dp | 在播放界面中的垂直位置偏移（正值向上）；全屏任意位置可调，偏移后点击仍有效 |
1
---
...
| 切换按钮上下偏移 | -800 ~ +800 dp | 0 dp | 调整切换按钮行在播放界面中的垂直位置；正值向上移，负值向下移；全屏任意位置可调，偏移后点击仍有效 |
...
| `skip_btn_offset_y` | int | 0 | 切换按钮垂直偏移（-800 ~ +800 dp） |
| `skip_btn_spacing` | int | 64 | 切换按钮间距（0–400 dp） |
| `seek_offset_y` | int | 0 | 跳转按钮垂直偏移（-800 ~ +800 dp） |
| `seek_btn_spacing` | int | 24 | 跳转按钮间距（0–300 dp） |
...
| 标签 | 内容 |
|------|------|
| 跳转 | 跳转时长（全局）→ **竖屏\|横屏** 子标签 → 按钮大小、透明度、上下偏移、按钮间距 |
| 切换 | **竖屏\|横屏** 子标签 → 按钮大小、透明度、上下偏移、按钮间距 |
| 暂停 | **新增** — 按钮大小、透明度、上下偏移 |
| 进度条 | 进度条粗细、已播放透明度 |
...
### 方格大小可调（媒体库 / 内置媒体 / 我的列表 分别调节）

在设置 → 列表模式中，分别为三个区域调节方格宽度（80–220 dp）：

- **媒体库**：`grid_cell_width_library`
- **内置媒体**：`grid_cell_width_builtin`
- **我的列表**：`grid_cell_width_playlist`（播放列表卡片 + 列表内媒体项共用）

值越小，每行格子越多；值越大，格子越大。列数由屏幕宽度和格子宽度自动计算，限制在 2–6 列。

### v16.4 修改记录

| 文件 | 修改内容 |
|------|---------|
| `activity_settings.xml` | 「方框格大小」改为 SeekBar（80–220 dp），替代原 2/3/4 列 RadioGroup |
| `SettingsActivity.java` | 新增 `PREF_GRID_CELL_WIDTH_DP`、`migrateGridSpanToCellWidth()`；SeekBar 绑定与保存 |
| `MainActivity.java` | 新增 `computeGridSpanCount()`，根据格子宽度和屏幕宽度计算列数；`applyViewMode()`、`refreshPlaylistDetail()` 使用新逻辑 |
| `strings.xml` | 更新 `settings_grid_size`、新增 `settings_grid_cell_width` |
...
**问题**：原先跳转按钮（前进/后退）和切换按钮（上一首/下一首）在同一垂直 LinearLayout 中，当跳转按钮上移时，切换按钮会消失（布局高度变负导致裁剪）。

**修复**：中央控制区改为 FrameLayout，seekRow 和 skipRow 作为独立子视图，各自根据屏幕中心 + 偏移量计算 `topMargin`，两行互不影响。

| 文件 | 修改内容 |
|------|---------|
| `activity_player.xml` | 中央区域由 LinearLayout 改为 FrameLayout（`centerControlsContainer`）；seekRow、skipRow 增加 `layout_gravity="top|center_horizontal"`，移除 skipRow 的 `layout_marginTop` |
| `PlayerActivity.java` | 新增 `applyCenterOffset()`，以屏幕中心为基准独立计算两行 `topMargin`；seek/skip 偏移合并为一次 `post()` 调用；移除原 skipRow 的 margin 逻辑 |
...
**问题**：竖屏上下滑动、横屏左右滑动切换视频功能无法触发，控件层可见或隐藏时均无效。

**根因**：
1. 控件叠加层（`controlsOverlay`）覆盖在 `playerView` 之上，当控件可见时触摸事件被叠加层接收，`playerView` 的 `OnTouchListener` 无法收到事件；
2. 仅依赖 `Activity.dispatchTouchEvent` 传递事件，部分设备或场景下可能无法可靠接收；
3. `getWindow().getDecorView().getWidth()` 在布局未完成时可能返回 0，导致 `actWindowWidth` 异常。

**修复**：

| 文件 | 修改内容 |
|------|---------|
| `PlayerActivity.java` | **`dispatchTouchEvent`**：当 `getDecorView().getWidth()` 为 0 时，使用 `getResources().getDisplayMetrics().widthPixels` 作为 fallback；**`setupGestures`**：在 `controlsOverlay` 上添加 `OnTouchListener`，将触摸事件转发到 `feedSwipeEvent` 并返回 `false` 以不消费事件，确保控件层可见时滑动仍能触发切换；统一使用 `SettingsActivity.PREFS_NAME` 和 `PREF_PORTRAIT_SWIPE` / `PREF_LANDSCAPE_SWIPE` 读取手势方向偏好 |

**效果**：
- 竖屏：上下滑动（VERTICAL 模式）或左右滑动（HORIZONTAL 模式）均可切换视频；
- 横屏：左右滑动（HORIZONTAL 模式）或上下滑动（VERTICAL 模式）均可切换视频；
- 控件层可见或隐藏时均能正常响应；
- 其他功能（单击显隐控件、双击暂停、长按快进、左侧亮度调节等）不受影响。
...
在设置 → 列表面板（与面板竖屏、面板横屏并排的第三个标签）中，可调节播放界面的播放列表面板外观：

| 设置项 | 范围 | 默认值 | 说明 |
|--------|------|--------|------|
| 背景透明度 | 0–100% | 87% | 面板背景的透明度，0 为全透明 |
| 宽度（左右滑出时） | 200–450 dp | 260 dp | 面板从左侧或右侧滑出时的宽度 |
| 高度（上下滑出时） | 200–550 dp | 300 dp | 面板从上方或下方滑出时的高度 |
| 视频方块大小 | 40–120 dp | 72 dp | 播放列表面板内每个条目的缩略图宽度，高度按 52/72 比例自动计算 |
| 列表排列方向 | 竖排 / 横排 | 竖排 | 视频条目在面板内的排列方式：竖排为纵向列表，横排为横向滚动 |

**偏好键**：
- `playlist_panel_bg_alpha`：背景透明度（int，0–100）
- `playlist_panel_width_dp`：左右滑出时的宽度（int，dp）
- `playlist_panel_height_dp`：上下滑出时的高度（int，dp）
- `playlist_panel_item_size_dp`：视频方块宽度（int，dp，40–120）
- `playlist_panel_orientation`：`VERTICAL` 或 `HORIZONTAL`

**实时预览**：从播放界面打开设置并调整上述参数时，若播放列表面板已打开，修改会实时反映。

### v16.7 修改记录

| 文件 | 修改内容 |
|------|---------|
| `SettingsActivity.java` | 新增 `PREF_PLAYLIST_PANEL_BG_ALPHA`、`PREF_PLAYLIST_PANEL_WIDTH_DP`、`PREF_PLAYLIST_PANEL_HEIGHT_DP`、`PREF_PLAYLIST_PANEL_ORIENTATION` 及默认值；generalTabRow 新增「列表面板」标签；sectionPlaylistPanel 含背景透明度、宽度、高度 SeekBar 及竖排/横排 RadioGroup |
| `activity_settings.xml` | 新增 tabPlaylistPanel、sectionPlaylistPanel（背景透明度、宽度、高度 SeekBar + 排列方向 RadioGroup） |
| `strings.xml` | 新增 `settings_playlist_panel_tab`、`settings_playlist_panel_bg_alpha`、`settings_playlist_panel_width`、`settings_playlist_panel_height`、`settings_playlist_panel_orientation`、`settings_playlist_panel_vertical`、`settings_playlist_panel_horizontal` |
| `PlayerActivity.java` | `applyPlaylistPanelLayout()` 从偏好读取宽高和背景透明度，动态设置 `playlistPanel` 背景色和 LayoutParams；`setupPlaylistPanel()` 根据 `PREF_PLAYLIST_PANEL_ORIENTATION` 设置 RecyclerView 的 LinearLayoutManager 方向；`applyPlayerSettings()` 中若面板已打开则实时更新布局和排列方向 |
...
在设置 → 播放控件 中，倍速按钮（如 "1×"）现支持与其它顶栏按钮相同的配置：

| 配置项 | 说明 |
|--------|------|
| **有无** | Switch 开关控制倍速按钮的显示/隐藏 |
| **顺序** | 在「顶栏」标签中通过 ↑↓ 调整倍速按钮在顶栏中的位置 |
| **颜色** | 点击色块弹出颜色选择对话框，可选白/红/橙/青/绿/黄 |

**偏好键**：
- `btn_speed_visible`：是否显示倍速按钮（boolean，默认 true）
- `btn_speed_color`：倍速按钮文字颜色（string：`white` / `accent` / `orange` / `cyan` / `green` / `yellow`）

**默认顶栏顺序**：`lock, play-mode, playlist-panel, speed, subtitle-toggle, subtitle, subtitle-list, rotate, settings`

### v16.8 修改记录

| 文件 | 修改内容 |
|------|---------|
| `SettingsActivity.java` | 新增 `PREF_BTN_SPEED_VISIBLE`、`PREF_BTN_SPEED_COLOR`；`DEFAULT_TOP_BTN_ORDER` 加入 `speed`；播放控件数组扩展倍速按钮项；`getTopBtnLabels()` 增加倍速标签 |
| `PlayerActivity.java` | 新增 `applySpeedButtonSettings()` 应用倍速按钮显隐与颜色；`applyButtonOrder()` 将 `tvSpeed` 纳入顶栏排序；旧配置自动迁移注入 `speed` |
| `strings.xml` | 新增 `settings_ctrl_speed`（"倍速按钮"） |

---

## v16.9 变更记录

### 长按 2.5× 提示可配置（有无、颜色、透明度）

在设置 → 按钮调试区 → **长按** 标签中，可调节长按屏幕激活 2.5× 快进时中央显示的「2.5× 快进中」图标：

| 配置项 | 说明 |
|--------|------|
| **有无** | Switch 开关控制长按快进时是否显示该提示图标 |
| **颜色** | 点击色块弹出颜色选择，可选白/红/橙/青/绿/黄 |
| **透明度** | SeekBar 调节 10–100%，默认 100% |

**偏好键**：
- `longpress_hint_visible`：是否显示（boolean，默认 true）
- `longpress_hint_color`：颜色（string，同其他按钮）
- `longpress_hint_alpha`：透明度（int 10–100，默认 100）

### v16.9 修改记录

| 文件 | 修改内容 |
|------|---------|
| `SettingsActivity.java` | 新增 `PREF_LONGPRESS_HINT_VISIBLE`、`PREF_LONGPRESS_HINT_COLOR`、`PREF_LONGPRESS_HINT_ALPHA`；按钮调试区新增「长按」标签及 sectionLongPress（Switch + 色块 + SeekBar）；`showLongPressHintColorPicker()` |
| `PlayerActivity.java` | 新增 `applyLongPressHintSettings()` 应用颜色与透明度；`onLongPressStart` 根据显隐设置决定是否显示提示 |
| `activity_settings.xml` | 新增 tabLongPress、sectionLongPress |
| `strings.xml` | 新增 `settings_longpress_hint_visible`、`settings_longpress_hint_color`、`settings_longpress_hint_alpha` |
...
从设置界面移除「竖屏手势」「横屏手势」两个标签及其配置区域。手势切换视频功能仍保留（竖屏默认上下滑、横屏默认左右滑），但用户无法再在设置中修改切换方向。

**变更范围**：仅设置模块，播放器手势逻辑未改动。

### v16.10 修改记录

| 文件 | 修改内容 |
|------|---------|
| `activity_settings.xml` | 移除 tabPortraitSwipe、tabLandscapeSwipe 及 sectionPortraitSwipe、sectionLandscapeSwipe；generalTabRow 仅保留面板竖屏、面板横屏、列表面板 |
| `SettingsActivity.java` | 移除竖屏/横屏手势相关变量、findViewById、标签切换逻辑及 RadioGroup 配置代码 |
...
在设置 → 列表面板 中新增「视频方块大小」SeekBar，可调节播放界面播放列表面板内每个条目的缩略图尺寸（40–120 dp，默认 72 dp）。高度按 52/72 比例自动计算，保持原有宽高比。设置变更后若面板已打开会实时生效。

### v16.11 修改记录

| 文件 | 修改内容 |
|------|---------|
| `SettingsActivity.java` | 新增 `PREF_PLAYLIST_PANEL_ITEM_SIZE_DP`、`DEFAULT_PLAYLIST_PANEL_ITEM_SIZE_DP`；sectionPlaylistPanel 新增 SeekBar 及标签 |
| `activity_settings.xml` | 新增 tvPlaylistPanelItemSizeLabel、sbPlaylistPanelItemSize |
| `strings.xml` | 新增 `settings_playlist_panel_item_size` |
| `item_player_playlist.xml` | 缩略图 FrameLayout 新增 id `framePanelThumb` |
| `PlayerPlaylistAdapter.java` | 构造函数新增 `thumbSizeDp` 参数；`setThumbSizeDp()` 支持设置变更时刷新；`onBindViewHolder` 根据尺寸动态设置 FrameLayout 宽高 |
| `PlayerActivity.java` | `setupPlaylistPanel()` 读取并传入 thumbSizeDp；`applyPlayerSettings()` 中调用 `panelAdapter.setThumbSizeDp()` 实时更新 |
...
在设置 → 播放控件 中新增「倍速外圈」选项，可调节倍速按钮（如 "1×"）外围描边/边框的**有无**（Switch 开关）和**颜色**（白/红/橙/青/绿/黄，默认红色）。

### v16.12 修改记录

| 文件 | 修改内容 |
|------|---------|
| `SettingsActivity.java` | 新增 `PREF_BTN_SPEED_STROKE_COLOR`、`PREF_BTN_SPEED_STROKE_VISIBLE`；播放控件列表新增「倍速外圈」行（有无 Switch + 颜色选择） |
| `PlayerActivity.java` | `applySpeedButtonSettings()` 读取外圈有无与颜色，动态创建 `GradientDrawable` 作为倍速 TextView 背景（stroke 宽度 0 或 1dp） |
| `strings.xml` | 新增 `settings_ctrl_speed_stroke`（"倍速外圈"） |
...
1. **播放控件移至字幕默认下方**：字幕面板、字幕默认为第一行；播放控件单独占第二行。
2. **设置页背景**：移除「设置页背景」折叠按钮，直接显示透明度 SeekBar。
3. **按钮调试区分行**：暂停、进度条、长按为第一行；跳转、切换为第二行（位于暂停行下方）。

### v16.14 修改记录

| 文件 | 修改内容 |
|------|---------|
| `activity_settings.xml` | 设置页背景改为直接显示；miscTabRow 仅含字幕面板+字幕默认；新增 playerControlsTabRow 单独一行放播放控件；btnTuneTabRow 拆为 btnTuneTabRow2（暂停/进度条/长按，第一行）和 btnTuneTabRow1（跳转/切换，第二行） |
| `SettingsActivity.java` | miscTabToggle 移除 tabSettingsBg，仅处理字幕面板、字幕默认、播放控件 |

---

*VideoMaster v16.14 — 纯本地，无联网，完全离线。*
...
**功能**：跳转按钮（前进/后退）和切换按钮（上一首/下一首）的所有布局相关设置均可分别针对竖屏和横屏独立配置。
...
| 透明度 | 独立 | 图标不透明度 |
| 上下偏移 | 独立 | 垂直位置（±800 dp） |
| 按钮间距 | 独立 | 跳转：与播放按钮间距；切换：两按钮间距（横屏默认 100 dp，竖屏默认 64 dp） |

**入口**：设置 → 按钮调试区 → 跳转 / 切换 → 竖屏 | 横屏 子标签切换。

**兼容**：若未单独配置竖屏/横屏，则沿用原有全局偏好；旋转屏幕时自动应用对应方向的设置。

### v16.16 修改记录

| 文件 | 修改内容 |
|------|---------|
| `SettingsActivity.java` | 新增 `*_PORTRAIT` / `*_LANDSCAPE` 偏好键；跳转/切换折叠面板内增加竖屏\|横屏子标签及双份 SeekBar；读取时若无新键则回退到旧键 |
| `PlayerActivity.java` | `applyPlayerSettings()` 按当前方向读取对应偏好；`onConfigurationChanged` 中调用 `applyPlayerSettings()` 以在旋转时刷新 |
| `activity_settings.xml` | 跳转/切换面板内增加竖屏\|横屏标签行及 `sectionSeekPortrait/Landscape`、`sectionSkipPortrait/Landscape` 双份 SeekBar |
...
**问题**：横屏时跳转按钮（前进/后退）与切换按钮（上一首/下一首）两行之间的垂直间距仅 4 dp，过小易误触、视觉拥挤。

**修改**：针对横屏下「左右间距不够、上下移动太大」及「不需要单独设置两行间距」的反馈：

1. **移除两行固定间距**：跳转行与切换行之间不再有固定 gap，仅由各自的上下偏移控制位置；切换行紧贴跳转行下方，其上下偏移可间接调节两行间距
2. **左右间距上限**：横屏跳转 0–300 dp、切换 0–400 dp
3. **上下偏移范围**：横屏 ±200 dp

| 文件 | 修改内容 |
|------|---------|
| `PlayerActivity.java` | `applyCenterOffset()` 移除 `gapPx`，`skipTop = centerY - seekH/2 + seekH - skipOffPx`；横屏偏移 clamp 到 ±200 |
| `SettingsActivity.java` | 横屏跳转间距 max 300、切换 max 400；横屏上下偏移 ±200 |

### 竖屏按钮间距上限提高

**问题**：竖屏下跳转按钮和切换按钮的最大间距（原 200 dp / 300 dp）仍不够大，无法满足大屏或单手操作需求。

**修改**：竖屏与横屏统一提高间距上限：
- **跳转按钮间距**：0–300 dp（原 200 dp）
- **切换按钮间距**：0–400 dp（原 300 dp）

| 文件 | 修改内容 |
|------|---------|
| `activity_settings.xml` | 跳转/切换竖屏、横屏 SeekBar `android:max` 分别改为 300、400 |
| `SettingsActivity.java` | `sbSeekSpacingP/L`、`sbSkipSpacingP/L` 的 `setMax()` 及 `Math.min()` 同步更新 |
...
**功能**：播放界面中央控制区的两行按钮间距可在设置中调节，避免误触、便于单手操作。

| 设置项 | 范围 | 默认值 | 说明 |
|--------|------|--------|------|
| 跳转按钮间距 | 0–300 dp | 24 dp | 前进/后退按钮与播放按钮之间的间距 |
| 切换按钮间距 | 0–400 dp | 64 dp | 上一首/下一首按钮之间的间距 |

**入口**：设置 → 按钮调试区 → 跳转 / 切换 标签 → 各自折叠面板内的「按钮间距」SeekBar。

### v16.15 修改记录

| 文件 | 修改内容 |
|------|---------|
| `SettingsActivity.java` | 新增 `PREF_SEEK_BTN_SPACING` / `PREF_SKIP_BTN_SPACING` 常量及默认值；跳转/切换折叠面板内新增按钮间距 SeekBar 及监听逻辑 |
| `PlayerActivity.java` | `applyPlayerSettings()` 中读取间距设置，通过 `MarginLayoutParams.setMarginStart/setMarginEnd` 应用到播放按钮、切换按钮 |
| `activity_player.xml` | 移除播放按钮、切换按钮的硬编码 margin，改由代码根据设置动态设置 |
| `activity_settings.xml` | 跳转按钮、切换按钮折叠面板内新增「按钮间距」TextView + SeekBar |
| `strings.xml` | 新增 `settings_seek_btn_spacing`、`settings_skip_btn_spacing` |