...
| **倍速按钮可配置** | 设置中可控制倍速按钮的显隐、文字颜色、**外圈颜色**（白/红/橙/青/绿/黄）、顶栏排序位置 |
...
在设置 → 播放控件 中新增「倍速外圈」颜色选项，可调节倍速按钮（如 "1×"）外围描边/边框的颜色。与其它按钮颜色相同，可选白/红/橙/青/绿/黄，默认红色（accent）。

### v16.12 修改记录

| 文件 | 修改内容 |
|------|---------|
| `SettingsActivity.java` | 新增 `PREF_BTN_SPEED_STROKE_COLOR`；播放控件列表新增「倍速外圈」行（仅颜色选择，无显隐开关） |
| `PlayerActivity.java` | `applySpeedButtonSettings()` 读取外圈颜色，动态创建 `GradientDrawable` 作为倍速 TextView 背景（含 stroke） |
