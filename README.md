...
| **播放列表面板外观可配置** | 设置中可调节面板背景透明度（0–100%）、宽度（左右滑出时 200–450 dp）、高度（上下滑出时 200–550 dp）、列表排列方向（竖排/横排） |
| **播放控件全面可配置** | 设置中可控制每个播放按钮的显隐和颜色（白/红/橙/青/绿/黄），含播放界面内置设置入口 |
| **倍速按钮可配置** | 设置中可控制倍速按钮的显隐、颜色（白/红/橙/青/绿/黄）、顶栏排序位置 |
...
在设置 → 列表面板（与竖屏手势、横屏手势、面板竖屏、面板横屏并排的第五个标签）中，可调节播放界面的播放列表面板外观：
...
*VideoMaster v16.9 — 纯本地，无联网，完全离线。*