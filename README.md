...
无需长按，**直接滑动**即可切换媒体。竖屏和横屏分别可设置滑动方向（在主界面右上角三点菜单→设置中配置）。

#### 竖屏模式（默认：上下滑）
| 手势 | 动作 |
|------|------|
| 右侧向下滑（无需长按） | 切换到**下一个**媒体 |
| 右侧向上滑（无需长按） | 切换到**上一个**媒体 |
| 左侧竖向滑动 | 调节屏幕亮度（上↑亮，下↓暗） |
...
#### 左侧亮度区域说明
- 左半屏竖向滑动**始终**用于调节亮度，不触发媒体切换。
- 当竖屏/横屏选择「上下滑」切换模式时，媒体切换仅在**右半屏**触发，左侧保留给亮度。
...
- **图标已移除**：亮度指示器只显示竖向进度条 + 百分比数字，不再有太阳图标遮挡画面。
- **触发区域**：屏幕左半部分纵向滑动触发（无需长按，两种屏幕方向均有效）。
...
| 文件 | 用途 |
|------|------|
| `res/drawable/seekbar_progress.xml` | 自定义粗进度条轨道（8dp 高，红色，圆角） |
| `res/drawable/seekbar_thumb.xml` | 自定义滑块（22dp 实心红圆） |
| `res/menu/menu_main.xml` | 主界面工具栏菜单（含"设置"入口） |

---

*VideoMaster v4.0.0 — 纯本地，无联网，完全离线。*
